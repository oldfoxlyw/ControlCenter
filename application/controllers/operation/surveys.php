...
class Surveys extends CI_Controller {
	...
	private $permissionName = 'operation_survey';
	...
	public function index() {
		 ...
		 * @novar
		 */
		$rowTotal = $this->survey->getSurveyTotal();
		...
		$result = $this->survey->getSurvey(null, $itemPerPage, $offset);
	...
	public function action() {
		...
		switch($action) {
			case 'delete':
				$this->survey->deleteSurvey($surveyId);
				$this->survey->deleteQuestion(array(
					'survey_id'	=>	$surveyId
				));
				$this->logs->write(array(
					'log_type'	=>	'SCC_SURVEY_DELETE'
				));
				break;
		}
